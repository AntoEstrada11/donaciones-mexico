...
7. Privacidad: registro y donación exigen consentimiento expreso, que Nitro valida y guarda en `consents`. El titular ejerce sus derechos ARCO desde `/perfil` (ver [/data/personal-data-inventory.md](/data/personal-data-inventory.md)).

# Componentes

| Pieza | Rol |
|-------|-----|
| `pages/` | Rutas UI: `/`, `/iglesias`, `/donaciones`, `/historial`, `/login`, `/registro`, `/perfil`, `/spei`, `/privacidad`, `/terminos`, `/admin` |
