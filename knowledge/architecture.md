...
4. Iglesias: Nitro (`GET /api/churches`) consulta WordPress IURD; si falla, sirve `server/data/churches.json`. El cliente usa `useChurches`. Las donaciones guardan el identificador (externo o de muestra).
...
| Pieza | Rol |
|-------|-----|
| `pages/` | Rutas UI: `/`, `/iglesias`, `/donaciones`, `/historial`, `/login`, `/registro`, `/perfil`, `/spei`, `/admin` |
...
| `server/utils/churches.ts` | Proxy WordPress + fallback `churches.json` |
