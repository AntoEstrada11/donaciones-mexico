...
Sitio web Nuxt 3 para que donantes en México registren cuenta, elijan iglesia y campaña, y generen donaciones (SPEI, tarjeta vía MercadoPago y PayPal). Inspirado en [doar.universal.org](https://doar.universal.org). Los datos de donantes y donaciones viven en una base PostgreSQL propia; ya no hay integración con Odoo para esos datos.
...
| Cobros | MercadoPago (principal) + PayPal; checkout alojado; secretos en env. Ver [/integrations/](/integrations/) |
...
Fase funcional: registro, login, perfil (con ARCO), directorio de iglesias, alta de donaciones, checkout MP/PayPal, historial, panel admin (**Personalizar**: carrusel, pie/SPEI y cobros), tutorial SPEI (`/spei`) y páginas legales (`/privacidad`, `/terminos`), persistido en PostgreSQL. Sin donación recurrente ni emisión de CFDI. Pendiente jurídico: confirmar domicilio y buzón ARCO en `utils/legal.ts`. Backlog: [/roadmap.md](/roadmap.md).
